...
class A09 {
	//lt부터 rt사이에 정답이 있는 경우에만 결정 알고리즘을 적용할 수 있다.
	
	int count(int[] arr, int capacity) {	
		int dvdCnt = 1, timeSum = 0;
		for(int x : arr) {
			//담을 수 있는 용량을 넘어가면
			if(timeSum+x > capacity) {
				dvdCnt++;
				timeSum = x;
			//용량이 남아 있으면
			}else timeSum += x;
	...
	int solution(int n, int m, int[] arr) {
		int answer = 0;
		//간단하게 최대값 구하기 위해 스트림 사용, 배열에서 스트림을 얻으려면 아래와 같이
		//max의 리턴값이 int가 아니기 때문에 getAsInt를 사용
